...
public class FogController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            ClearFog(unit.transform);

            UpdateColors();
        }
    }

    void ClearFog(Transform location)
    {
        for (int i = 0; i < vertices.Length; i++)
        {
            Vector3 v = fogPlane.transform.TransformPoint(vertices[i]);

            float distance = Vector3.SqrMagnitude(v - (new Vector3(location.position.x, v.y, location.transform.position.z)));
